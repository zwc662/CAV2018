...
4. There may be slight variation between the result in log file and the data in the paper. This is largely due to the randomization in the algorithm. It is normal that differences in number of iterations, average step length are less than 3. However, if serious inconsistency happens, please run the program for at least one more time.

5. The data in the paper uses the precision of one decimal. Don't recommend user to try the p*=0.0% because the safest policy may has less than 0.1% unsafe probability. 
