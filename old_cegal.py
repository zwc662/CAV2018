...
class cegal(apirl, object):
    ...
    def iteration(self, exp_mu = None, opt = None, safety = None, steps = None):
        ...
        if opt is not None:
            ...
            opt['prob'] = prob
            if prob < safety:
                print("Provided policy is safe: %f < %f. Use as initial safe policy" % (prob, safety))
                features['cands'].append(mu.copy())
                features['safes'].append(mu.copy())
            else:
                print("Provided policy is unsafe: %f > %f. Generating initial safe policy" % (prob, safety))        
                if cex is not None:
 ...
 + str(safety_) + ". Lower down the safety threshold.")
	...
	for path_string in range(len(path_strings)):
	    ...
	    s = paths[path][state]
            diff = float('inf')
	    while diff  > epsilon and steps >= length:
	        #mu_path = mu_path + (gamma**(length - 1)) * mus[s]
                #break
	        mu_path = mu_path + (gamma**(length + 0)) * self.M.features[s]
		length = length + 1
                diff = np.linalg.norm(self.M.features[s] * gamma**length) 
