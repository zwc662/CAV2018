	...
	static final public void run() throws InterruptedException, FileNotFoundException {
		try {
			...
			PrintStream ps_file = new PrintStream(new FileOutputStream(
					new File("//home/zekunzhou/workspace/Safety-AI-MDP/prism-4.3.1-src/src/demos/grid_world.pm")));
			System.setOut(ps_file);
			...
			ModulesFile modulesFile = prism
					.parseModelFile(new File("//home/zekunzhou/workspace/Safety-AI-MDP/prism-4.3.1-src/src/demos/grid_world.pm"));
			prism.loadPRISMModel(modulesFile);
			// Parse and load a properties model for the model
			PropertiesFile propertiesFile = prism.parsePropertiesFile(modulesFile,
					new File("//home/zekunzhou/workspace/Safety-AI-MDP/prism-4.3.1-src/src/demos/grid_world.pctl"));
			Result result = prism.modelCheck(propertiesFile, propertiesFile.getPropertyObject(0));
			...
			for (int i = 0; i <= y_max; i++) {
				for (int j = 0; j <= x_max; j++) {
					...
					modulesFile = prism.parseModelFile(
							new File("//home/zekunzhou/workspace/Safety-AI-MDP/prism-4.3.1-src/src/demos/grid_world.pm"));
					prism.loadPRISMModel(modulesFile);
					propertiesFile = prism.parsePropertiesString(mf_opt, "P=? [true U<64 x = " + Integer.toString(x_l_0) + " & y = " + Integer.toString(y_l_0) + "]");
			...
			for (int i = 0; i <= y_max; i++) {
				for (int j = 0; j <= x_max; j++) {
					...
					modulesFile = prism.parseModelFile(
							new File("//home/zekunzhou/workspace/Safety-AI-MDP/prism-4.3.1-src/src/demos/grid_world.pm"));
					prism.loadPRISMModel(modulesFile);
					propertiesFile = prism.parsePropertiesString(mf_demo, "P=? [true U<64 x = " + Integer.toString(x_l_0) + " & y = " + Integer.toString(y_l_0) + "]");
					result = prism.modelCheck(propertiesFile, propertiesFile.getPropertyObject(0));
					// System.out.println(result.getResult());
